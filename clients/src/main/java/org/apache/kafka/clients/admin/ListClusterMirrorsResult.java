...
public class ListClusterMirrorsResult {
    ...
    ListClusterMirrorsResult(KafkaFuture<Collection<Object>> future) {
        ...
        future.thenApply(results -> {
            ...
            ArrayList<ClusterMirrorListing> curValid = new ArrayList<>();
            for (Object resultObject : results) {
                if (resultObject instanceof Throwable) {
                    curErrors.add((Throwable) resultObject);
                } else {
                    curValid.add((ClusterMirrorListing) resultObject);
                }
            }
            List<ClusterMirrorListing> validResult = Collections.unmodifiableList(curValid);
