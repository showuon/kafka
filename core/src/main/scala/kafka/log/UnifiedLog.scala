...
class UnifiedLog(@volatile var logStartOffset: Long,
  ...
  /**
   * Increment the "local log start offset" if the provided offset is larger,
   * and if remote log is disabled or onlyLocalLogStartOffsetUpdate is false, the "log start offset" is also updated.
   *
  ...
  private def deleteOldSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean,
    ...
    lock synchronized {
      val deletable = localLog.deletableSegments(shouldDelete, highestOffsetInRemoteStorage)
  ...
  private def deleteRetentionSizeBreachedSegments(): Int = {
    val retentionSize = localRetentionSize()
