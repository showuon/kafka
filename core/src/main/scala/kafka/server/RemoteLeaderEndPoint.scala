...
<<<<<<< HEAD
=======
>>>>>>> bb5eefee87 (Only reset partitions when the version crosses the v11/v12 boundary)
  ...
  override def buildFetch(partitions: java.util.Map[TopicPartition, PartitionFetchState]): ResultWithPartitions[java.util.Optional[ReplicaFetch]] = {
    ...
    val fetchRequestOpt = if (fetchData.sessionPartitions.isEmpty && fetchData.toForget.isEmpty) {
      ...
      val requestBuilder = if (isClusterMirror) {
        ...
        FetchRequest.Builder.forMirror(version, maxWait, minBytes, fetchData.toSend).isolationLevel(IsolationLevel.READ_UNCOMMITTED)
      } else {
    ...
    new ResultWithPartitions(fetchRequestOpt, partitionsWithError.asJava, util.Set.of(), util.Set.of())
  }
