...
import kafka.server.mirror.{MirrorFetcherManager, MirrorMetadataManager}
...
class ReplicaManager(val config: KafkaConfig,
  ...
  private def applyLocalLeadersDelta(
    ...
    // Stop both regular AND mirror fetchers when becoming a leader
  ...
  private def applyLocalFollowersDelta(
    ...
    localFollowers.foreachEntry { (tp, info) =>
      getOrCreatePartition(tp, delta, info.topicId).foreach { case (partition, isNew) =>
        try {
          ...
          val isNewLeaderEpoch = if (hasRemoteLeader) {
            // For remote leaders, check if leader epoch changed without calling makeFollower
            val currentLeaderEpoch = partition.getLeaderEpoch
            partition.updateLeaderEpochForRemoteLeader(state.leaderEpoch)
            state.leaderEpoch > currentLeaderEpoch
          } else {
            // For regular followers, call makeFollower which returns isNewLeaderEpoch
    ...
    if (partitionsToStartFetching.nonEmpty) {
      ...
      // Split partitions by cluster mirror name
