...
public final class ClusterMirrorConfig {
    ...
    public static final String SOURCE_CLUSTER_ID_DOC = CommonClientConfigs.SOURCE_CLUSTER_ID_DOC;
    ...
    public static final int FETCH_WAIT_MAX_MS_DEFAULT = 1000;
    public static final String FETCH_WAIT_MAX_MS_DOC = "The maximum wait time for each fetcher request issued by mirror replicas.";
    ...
    public static final String FETCH_MIN_BYTES_DOC = "Minimum bytes expected for each fetch response.";

    public static final String FETCH_RESPONSE_MAX_BYTES_CONFIG = "fetch.response.max.bytes";
    public static final int FETCH_RESPONSE_MAX_BYTES_DEFAULT = 10 * 1024 * 1024;
    public static final String FETCH_RESPONSE_MAX_BYTES_DOC = "Maximum bytes expected for the entire fetch response. Records are fetched in batches, " +
    ...
    public static final int FETCH_MAX_BYTES_DEFAULT = 1024 * 1024;
    public static final String FETCH_MAX_BYTES_DOC = "The number of bytes of messages to attempt to fetch for each partition.";

    public static final String SOCKET_TIMEOUT_MS_CONFIG = "socket.timeout.ms";
    public static final int SOCKET_TIMEOUT_MS_DEFAULT = 30 * 1000;
    public static final String SOCKET_TIMEOUT_MS_DOC = "The socket timeout for network requests.";

    public static final String SOCKET_RECEIVE_BUFFER_BYTES_CONFIG = "socket.receive.buffer.bytes";
    public static final int SOCKET_RECEIVE_BUFFER_BYTES_DEFAULT = 64 * 1024;
    public static final String SOCKET_RECEIVE_BUFFER_BYTES_DOC = "The socket receive buffer for network requests to the leader for replicating data.";

    // Filter configuration
    public static final String TOPIC_PROPERTIES_EXCLUDE_CONFIG = "topic.properties.exclude";
    public static final String TOPIC_PROPERTIES_EXCLUDE_DEFAULT =
    ...
    public static final String TOPICS_INCLUDE_DOC = "A comma-separated list of regex patterns for topic names to include in mirroring. "
    ...
    public static final String TOPICS_EXCLUDE_DEFAULT = "__.*";
    public static final String TOPICS_EXCLUDE_DOC = "A comma-separated list of regex patterns for topic names to exclude from mirroring "
    ...
    public static final String GROUPS_EXCLUDE_DOC = "A comma-separated list of regex patterns for consumer group IDs to exclude from offset synchronization "
            + "(this takes precedence over mirror.groups.include).";

    public static final String ACLS_INCLUDE_CONFIG = "acls.include";
    public static final String ACLS_INCLUDE_DEFAULT = "*";
    public static final String ACLS_INCLUDE_DOC = "A comma-separated list of ACL include rules. Each rule uses semicolon-separated fields: "
    ...
    public static ClusterMirrorConfig fromProperties(Properties properties) {
        ...
        return new ClusterMirrorConfig(config,
                config.getList(TOPIC_PROPERTIES_EXCLUDE_CONFIG),
                config.getList(TOPICS_INCLUDE_CONFIG),
                config.getList(TOPICS_EXCLUDE_CONFIG),
                config.getList(GROUPS_INCLUDE_CONFIG),
                config.getList(GROUPS_EXCLUDE_CONFIG),
                config.getList(ACLS_INCLUDE_CONFIG));
    }
    ...
    public ClusterMirrorConfig(AbstractConfig config) {
        ...
        this.topicPropertiesExcludePattern = MirrorUtils.compilePatternList(Arrays.asList(TOPIC_PROPERTIES_EXCLUDE_DEFAULT.split(",")));
        this.topicsIncludePattern = null;
        this.topicsExcludePattern = MirrorUtils.compilePatternList(List.of(TOPICS_EXCLUDE_DEFAULT));
        this.groupsIncludePattern = MirrorUtils.compilePatternList(List.of(GROUPS_INCLUDE_DEFAULT));
        this.groupsExcludePattern = null;
        this.aclIncludeRules = MirrorUtils.parseAclRules(List.of(ACLS_INCLUDE_DEFAULT));
    }
    ...
    public int fetchWaitMaxMs() {
        return config.getInt(FETCH_WAIT_MAX_MS_CONFIG);
    }

    public int fetchMinBytes() {
        return config.getInt(FETCH_MIN_BYTES_CONFIG);
    }

    public int fetchResponseMaxBytes() {
        return config.getInt(FETCH_RESPONSE_MAX_BYTES_CONFIG);
    }

    public int fetchMaxBytes() {
        return config.getInt(FETCH_MAX_BYTES_CONFIG);
    }

    public int socketTimeoutMs() {
        return config.getInt(SOCKET_TIMEOUT_MS_CONFIG);
    }

    public int socketReceiveBufferBytes() {
        return config.getInt(SOCKET_RECEIVE_BUFFER_BYTES_CONFIG);
    }
